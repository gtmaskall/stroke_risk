...
proxy for age. It is quite possible that smoking status has
...
Could we fill in the missing smoking status values?
We might certainly expect to mark all children under the age
of ten as *never smoked* and expect to usually being correct.
Sadly, we couldn't assume we'd be 100% right here! But at what
age would we expect people to take up smoking? Can we predict
smokers from other features?

## Can we model missing smoking_status values?

Model age by smoking (balance for smoking status)
And can we drive glmnet on this data?
